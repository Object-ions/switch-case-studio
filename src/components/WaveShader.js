...
import '../styles/components/waveShader.scss';
...
// Brand color ramp baked in: deep purple → $g6 → $g7 → warm peach → $g1
// Hex values converted to normalized RGB for the shader.
// #2a1a4d (deep purple base) → #d99cff ($g6) → #f0d7ff ($g7) → #ffb38a (peach) → #ff834a ($g1)
const BRAND_PALETTE = [
  [0.165, 0.102, 0.302], // #2a1a4d
  [0.851, 0.612, 1.0], // #d99cff ($g6)
  [0.941, 0.843, 1.0], // #f0d7ff ($g7)
  [1.0, 0.702, 0.541], // #ffb38a
  [1.0, 0.514, 0.29], // #ff834a ($g1)
];

const WaveShader = forwardRef(({ className, initialParams }, ref) => {
  const hostRef = useRef(null);
  const paramsRef = useRef({ ...DEFAULTS, ...(initialParams || {}) });
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        u_dark: { value: 1 }, // Hardcoded dark — site is dark-mode only

        u_c0: { value: [...BRAND_PALETTE[0]] },
        u_c1: { value: [...BRAND_PALETTE[1]] },
        u_c2: { value: [...BRAND_PALETTE[2]] },
        u_c3: { value: [...BRAND_PALETTE[3]] },
        u_c4: { value: [...BRAND_PALETTE[4]] },
      },
    });
  ...
  return <div ref={hostRef} className={`wave-shader ${className || ''}`.trim()} />;
});
